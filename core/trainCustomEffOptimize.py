...
def Train(args, FeatExtor, DepthEstor, FeatEmbder,
    ...
    if args.optimizer is 'adam':
        ...
        scheduler_embd = lr_scheduler.ReduceLROnPlateau(optimizer_embd, mode='min', factor=0.7, patience=3, verbose=True)
        scheduler_extr = lr_scheduler.ReduceLROnPlateau(optimizer_extr, mode='min', factor=0.7, patience=3, verbose=True)
        scheduler_dep = lr_scheduler.ReduceLROnPlateau(optimizer_dep, mode='min', factor=0.7, patience=3, verbose=True)                                   
        ...
        for step in range(iternum):
            ...
            if (step+1) % args.log_step == 0:
                errors = OrderedDict([
                                    ...
                                    ('LR_extr', optimizer_extr.param_groups[0]['lr']),
                                    ('LR_embd', optimizer_embd.param_groups[0]['lr']),
                                    ('LR_dep', optimizer_dep.param_groups[0]['lr']),
                                    ])
            ...
            '''
        scheduler_extr.step(Loss_cls_train)
        scheduler_embd.step(Loss_cls_train)  
        scheduler_dep.step(Loss_dep_train)   
