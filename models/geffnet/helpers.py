...
def load_pretrained(model, url, filter_fn=None, strict=False):
    if not url:
    ...
    in_chans = model.in_chans#getattr(model, input_conv).weight.shape[1]
    num_classes = model.num_classes#getattr(model, classifier).weight.shape[0]
